...
if (isset($_POST['name'])) {
    ...
    $query = "INSERT INTO `studentbooking`( `name`, `email`, `reg_id`, `password`, `contactNumber`, `gender`,`mess`,`seater`,`fees`,`roomNumber`,`stayfrom`,`hostel_id`) VALUES 
    ('$name','$email','$registrationNumber','$password','$phone_no','$gender','$mess','$seater','$fees','$roomNumber','$date','$id')";
    if (mysqli_query($mysqli, $query)) {
        $student_id = $mysqli->insert_id;
        $query = "INSERT INTO `bill`( `student_id`, `hostel_id`, `start_from`, `bill`, `month`, `status`) VALUES 
                                ('$student_id','$id','$date','$fees','$month','paid')";
        ...
        echo "<script>alert('room is successfully registered')</script>";
    } else {
        echo $query;
    }
}
        ...
        <div class="container col-8">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group mt-3">
                                        <select class="form-control field required" name="roomNumber" id="room" onChange="getSeater(this.value);" required id="inlineFormCustomSelect">
                                            ...
                                            $query = "SELECT * FROM `rooms` where `hostel_id` = '$id' AND `occupied`<`seater`";
                                            $stmt2 = $mysqli->prepare($query);
                                            $stmt2->execute();
                                            $res = $stmt2->get_result();
                                            //  echo "<option selected>Select...</option>";
                                            if($res->num_rows==0){
                                                echo "<option style='color:red' selected>Sorry! All rooms are booked</option>";
                                            }
                        ...
                        <div class="col-sm-12 col-md-6 col-lg-6">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="form-group mt-3">
                                        <input type="text" id="seater" onchange="roomseater(this.value)" name="seater" placeholder="Enter Seater No." class="form-control field " disabled>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <script>
                function submitform() {
                    ...
                    if (!$('#name').val() && !$('#email').val() && !$('#registrationNumber').val() && !$('#seater').val() && !$('#mess').val() && !$('#fpm').val()) {
